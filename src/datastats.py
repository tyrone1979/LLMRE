...
from data_chemdisgene import Relation, parse_pubtator_to_dict, parse_relationships_opened_file
...
def get_counts(pbtr_file: str, ctd_relns_file: str, new_relns_file: str = None):
    ...
    for doc in docs_dict.values():
        for men in doc.mentions:
            counts["n_mentions"][men.type] += 1
            if men.is_unresolved_mention():
                counts["n_unlinked_mentions"][men.type] += 1
            else:
                counts["n_linked_mentions"][men.type] += 1

        unique_mentioned_entities |= doc.get_mentioned_entities()

        for reln in doc.relations:
            r_label = reln.r
        ...
        if len(doc.relations) == 0:
            ...
            if not any(r.from_ctd for r in doc.relations):
                n_docs_no_ctd_relns += 1

            unique_relns |= set(get_reln_key(reln) for reln in doc.relations)
...
def get_reln_key(reln: Relation):
    return reln.r, reln.subj_type, reln.h, reln.obj_type, reln.t
...
def get_pretty_reln_label(relation_label):
    if relation_label == "Total":
        ...
        return Relation("Dummy", "Dummy", relation_label).get_pretty_relation_label()


def print_stats_ctd_derived(data_dir="../data/chemdisgene"):
...
def print_stats_curated(data_dir="../data/chemdisgene"):

    print(f"Processing {data_dir} ...", flush=True)

    ctd_relns_file = f"{data_dir}/approved_relns_ctd_v1.tsv"
    new_relns_file = f"{data_dir}/approved_relns_new_v1.tsv"
    if not os.path.exists(ctd_relns_file):
        ctd_relns_file = f"{data_dir}/approved_relns_ctd_v0.tsv"
        new_relns_file = f"{data_dir}/approved_relns_new_v0.tsv"

    stats = get_counts(f"{data_dir}/abstracts.txt", ctd_relns_file, new_relns_file)
...
def pp_nrels_by_ndocs_curated(data_dir="../data/curated"):
    ...
    all_reln_counts = Counter([len(doc.relations)
                               for doc in docs_dict.values()])
    ctd_reln_counts = Counter([len([r for r in doc.relations if r.from_ctd])
                               for doc in docs_dict.values()])
    new_reln_counts = Counter([len([r for r in doc.relations if not r.from_ctd])
                               for doc in docs_dict.values()])
